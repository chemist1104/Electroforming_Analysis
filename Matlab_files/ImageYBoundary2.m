...
for voltage=1:1:length(Voltageset)
    inputfile=strcat('figures/Cutimage/Cutimage', string(Voltageset(1,voltage)), 'V-Tot2intermediate-resized.dat');
...
%    minfY=LineY_min;
    ...
    dlmwrite(outputfile,boundaryline);
    
    dataarray = imresize(dataarray, [fix(lenT/Voltagefps{voltage}) fix((lenX/3)*0.24767)]);
    imagesc(dataarray);
    hold on
    axis xy
    plot(boundaryline(:,1)*0.24767, boundaryline(:,2)/Voltagefps{voltage}, 'w')
    saveas(gcf, sprintf('figures/Cutimage/processed/%0.1fV-resize2.png', Voltageset{voltage}));
